      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Hearings, deadlines, and events for this case. Court hearings also
      ...
      {dayEvents.length === 0 && (
        <div className="border-b border-white/[0.06] px-4 py-6 text-center">
          <p className="text-sm text-muted-foreground">
            No events on {format(anchor, "MMMM d, yyyy")}.
          </p>
        </div>
      )}
      {HOURS.map((hour) => {
        ...
        {days.map((day) => {
          ...
          return (
            <button
              ...
              onClick={() => onSelectDay(day)}
              className={cn(
                "min-h-[6.5rem] border-b border-r border-white/[0.05] p-1.5 text-left transition-colors hover:bg-white/[0.03]",
                !inMonth && "bg-black/20",
                isSelected && inMonth && "bg-white/[0.04]",
              )}
            >
